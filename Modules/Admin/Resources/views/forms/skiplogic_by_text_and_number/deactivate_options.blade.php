    ...
    @if(count($options_name) >= 2)
        ...
        <tr>
            <td style="text-align: center;width:15%;">
               <input type="checkbox" name="deactivate_options[{{$index}}][]" value="{{$options_value[$key]}}<<=!=>>{{$questions->id}}" class="deactivate_option_{{$questions->id}}{{$key}}_{{$index}}" onclick="disabled_opposite('{{$questions->id}}{{$key}}','activate_option_','{{$index}}','deactivate_option_');">
            </td>
